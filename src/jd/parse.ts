...
export type JdArea = { kind: "area"; area: number };
export type JdCategory = { kind: "category"; category: string };
export type JdId = { kind: "id"; category: string; id: string; extension?: "+" };
...
export function parseJdNumber(input: string): JdNumber | null {
  ...
  if (area) {
    ...
    return { kind: "area", area: Number(area[1]) * 10 };
  }

  const cat = CATEGORY.exec(s);
  if (cat) return { kind: "category", category: cat[1] };
...
/** Prefix at the start of a file or folder name: `10-19`, `11`, `11.11`, `11.11+`, followed by a separator or the end. */
const PREFIX = /^(\d0-\d9|\d{2}\.\d{1,2}\+?|\d{2})(?:[\s_-]+|$)/;
...
/** `AC.ID Title` with the title made safe for every file system and for Obsidian Sync. */
export function jdexNoteName(id: string, title: string): string {
  const safe = title
    .replace(/:/g, " -")
    .replace(/[*?"<>|/\\]/g, "")
    .replace(/\s+/g, " ")
    .trim();
  return safe === "" ? id : `${id} ${safe}`;
}
